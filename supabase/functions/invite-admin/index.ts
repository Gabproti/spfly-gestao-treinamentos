...
export default {
  fetch: withSupabase({ auth: 'user' }, async (request, ctx) => {
    ...
    if (!callerId) return Response.json({ error: 'Sessão inválida.' }, { status: 401 })

    const { data: admin, error: adminError } = await ctx.supabase.from('admin_users')
      .select('id').eq('id', callerId).eq('active', true).maybeSingle()
    if (adminError || !admin) return Response.json({ error: 'Somente administradores podem enviar convites.' }, { status: 403 })
    ...
    const { error: insertError } = await ctx.supabaseAdmin.from('admin_users').insert({
      id: invited.user.id, email, invited_by: callerId, active: true,
    })
    if (insertError) {
